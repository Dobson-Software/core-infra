...
module "eks" {
  ...
  eks_managed_node_groups = {
    default = {
      instance_types = var.node_instance_types != null ? var.node_instance_types : (var.environment == "prod" ? ["t3.xlarge"] : ["t3.medium"])
      capacity_type  = var.capacity_type
      min_size       = coalesce(var.node_min_size, var.environment == "prod" ? 3 : 2)
      max_size       = coalesce(var.node_max_size, var.environment == "prod" ? 12 : 6)
      desired_size   = coalesce(var.node_desired_size, var.environment == "prod" ? 4 : 3)
...
resource "aws_iam_role" "cluster_autoscaler" {
  name               = "cobalt-${var.environment}-cluster-autoscaler"
...
resource "aws_iam_role" "lb_controller" {
  name               = "cobalt-${var.environment}-lb-controller"
...
resource "aws_iam_role" "eso" {
  name               = "cobalt-${var.environment}-external-secrets"
...
module "ebs_csi_irsa" {
  ...
  role_name             = "cobalt-${var.environment}-ebs-csi-driver"
